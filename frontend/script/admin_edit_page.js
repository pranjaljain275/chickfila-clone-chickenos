...
async function addtoserver(obj) {
  try {
    let add_data = await fetch("http://localhost:275/meals/create", {
      method: "POST",
      headers: {
...
async function editData(id) {
  try {
    let edit_data = await fetch(`http://localhost:275/meals/${id}`);
    if (edit_data.ok) {
...
async function finalData(obj) {
  try {
    let edit_fetch = await fetch(`http://localhost:275/meals/${dataId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `${accesstokenAdmin}`,
