...
async function addtoserver(obj) {
  try {
    let add_data = await fetch("https://chickenos-chickfila-be.onrender.com/meals/create", {
      method: "POST",
      headers: {
...
async function editData(id) {
  try {
    let edit_data = await fetch(`https://chickenos-chickfila-be.onrender.com/meals/${id}`);
    if (edit_data.ok) {
...
async function finalData(obj) {
  try {
    let edit_fetch = await fetch(`https://chickenos-chickfila-be.onrender.com/meals/${dataId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `${accesstokenAdmin}`,
